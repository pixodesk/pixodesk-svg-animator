...
export const CASE_SECTIONS: Array<ExplorerSection> = [
    ...
    },
    {
        ...
        data: [
            ...
            { id: 'attr.transform.rotate', description: 'Breaks if rotation **wraps/normalizes** the angle — `0→540°` must spin 1.5 turns, not snap to 180°.', get doc() { return require('./cases/attrTransformRotate.svga').attrTransformRotate; } },
            ...
            { id: 'attr.transform.origin', description: 'Breaks if the transform **origin isn\'t animatable** — the spin must re-center as the pivot moves, not rotate about a fixed point.', get doc() { return require('./cases/attrTransformOrigin.svga').attrTransformOrigin; } },
    ...
    },
    {
        ...
        data: [
            { id: 'attr.color.fill', description: 'Breaks if flat color interpolates in the **wrong space** — `blue→red` should pass through a clean midpoint, not a muddy one.', get doc() { return require('./cases/attrColorFill.svga').attrColorFill; } },
            { id: 'attr.color.stroke', description: 'Same color-interpolation check on the **stroke** channel — catches stroke paint wired differently from fill.', get doc() { return require('./cases/attrColorStroke.svga').attrColorStroke; } },
        ],
    },
    {
        title: '§ 1.5 attr.gradient',
        data: [
            { id: 'attr.gradient.fill.linear', description: 'Breaks if **gradient stop colors don\'t animate** (geometry-only support) — the linear fill stalls.', get doc() { return require('./cases/attrGradientFillLinear.svga').attrGradientFillLinear; } },
            ...
            { id: 'attr.gradient.endpoints.radial', description: 'Breaks if the **focal point (`focal`)** offset is dropped — the highlight sits centered instead of off-center.', get doc() { return require('./cases/attrGradientEndpointsRadial.svga').attrGradientEndpointsRadial; } },
            ...
            { id: 'attr.gradient.objectBoundingBox', description: 'Breaks if `gradientUnits` isn\'t honored — `objectBoundingBox` (0–1) coords render at userSpace scale, blowing the gradient out of the shape.', get doc() { return require('./cases/attrGradientObjectBoundingBox.svga').attrGradientObjectBoundingBox; } },
        ],
    },
    {
        title: '§ 1.6 attr.path',
        data: [
            ...
            { id: 'attr.path.fillRule', description: 'Breaks if **`fillRule`** round-trips wrong — `evenodd` renders solid (no hollow center) on the self-intersecting path.', get doc() { return require('./cases/attrPathFillRule.svga').attrPathFillRule; } },
    ...
    },
    {
        ...
        data: [
            { id: 'attr.appearance.mixBlendMode', description: 'Breaks if `mix-blend-mode` isn\'t composited — each circle shows its own flat blue instead of a blended color. **It\'s a CSS-only property**: the player must apply it via `element.style`, not `setAttribute` (which the browser ignores), and the blend shape must sit **on top** of an opaque backdrop.', get doc() { return require('./cases/attrAppearanceMixBlendMode.svga').attrAppearanceMixBlendMode; } },
    ...
    },
    {
        ...
        data: [
            ...
            { id: 'elem.text', description: 'Breaks if **multi-line text or per-span color** is flattened — lines merge or lose color.', get doc() { return require('./cases/elemText.svga').elemText; } },
            ...
            { id: 'elem.text.anchor', description: 'Breaks if **`text-anchor`** start/middle/end isn\'t honored — rows won\'t align to the guide.', get doc() { return require('./cases/elemTextAnchor.svga').elemTextAnchor; } },
            ...
            { id: 'elem.filter.colorMatrix', description: '**Not built yet** — will guard an `feColorMatrix` recolor once implemented (see COVERAGE-GAPS.md).', get doc() { return require('./cases/elemFilterColorMatrix.svga').elemFilterColorMatrix; } },
            ...
            { id: 'elem.marker.orient', description: 'Breaks if **`orient="auto-start-reverse"`** or a fixed angle isn\'t honored — the start marker faces the wrong way.', get doc() { return require('./cases/elemMarkerOrient.svga').elemMarkerOrient; } },
            { id: 'elem.text.browser', description: 'Breaks if **multi-line text or per-span color** is flattened — lines merge or lose color.', get doc() { return require('./cases/elemTextBrowser.svga').elemTextBrowser; } },
    ...
    },
    {
        ...
        data: [
            { id: 'effect.retime.start', description: 'Breaks if a **materialized clone** holding a nested `<use>` loses its external **animated** content.', get doc() { return require('./cases/effectRetimeStart.svga').effectRetimeStart; } },
            { id: 'effect.retime.stretch', description: 'Breaks if a **materialized clone** holding a nested `<use>` loses its external **animated** content.', get doc() { return require('./cases/effectRetimeStretch.svga').effectRetimeStretch; } },
        ],
    },
    {
        ...
        data: [
            ...
            { id: 'effect.clone.contentRef', description: 'Breaks if the **content-ref** (group target) isn\'t materialized — the used group is empty.', get doc() { return require('./cases/effectCloneContentRef.svga').effectCloneContentRef; } },
    ...
    },
    {
        ...
        data: [
            ...
            { id: 'anim.config.direction', description: 'Breaks if **`direction=alternate`** isn\'t honored — playback won\'t reverse each iteration.', get doc() { return require('./cases/animConfigDirection.svga').animConfigDirection; } },
