...
/**
 * Run the FULL engine-aware pipeline (`materializeAllInTree`): effects + loop
 * expansion, plus the `waapi`-only stages (motion-path flatten + animated-`<use>`
 * inline). Use this to test behavior that DIFFERS between `waapi` and `frames`
 * — the plain {@link materialize} (effects only) is engine-agnostic.
 */
export function materializeEngine(input: PxNode, engine: PxTimelineEngine): PxNode {
    return materializeAllInTree(input as PxAnimatedSvgDocument, engine) as unknown as PxNode;
}
...
/**
 * Snapshot-normalizer → returns a STRING (not an object) so we control key order:
 ...
 */
export function normalizeGeneratedIds(tree: PxNode): string {
    ...
    // keys sorted (stable) and `children` appended last, then serialize.
...
/** Nodes that STILL carry `effects.<key>` after materialization (should be 0 —
